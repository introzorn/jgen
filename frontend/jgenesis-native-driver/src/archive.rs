...
pub enum ArchiveError {
    ...
    #[error("File '{file_name}' in archive '{path}' does not have a supported extension")]
    UnsupportedInnerFile { path: String, file_name: String },
}
...
/// A ROM path that may reference a specific file inside a .zip/.7z archive.
///
/// Syntax: `path/to/archive.7z#Inner ROM (U) [!].gen`
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ArchivePathSpec {
    pub archive_path: PathBuf,
    pub inner_file: Option<String>,
}

/// Split `archive.zip#game.gen` into archive path and optional inner file name.
///
/// If the path contains no `#`, or the part before `#` is not a .zip/.7z file, the full path is
/// returned unchanged with no inner file.
#[must_use]
pub fn parse_archive_path(path: &Path) -> ArchivePathSpec {
    let path_str = path.to_string_lossy();
    let Some(hash_idx) = path_str.rfind('#') else {
        return ArchivePathSpec { archive_path: path.to_path_buf(), inner_file: None };
    };

    let archive_str = &path_str[..hash_idx];
    let inner_str = &path_str[hash_idx + 1..];
    if inner_str.is_empty() {
        return ArchivePathSpec { archive_path: path.to_path_buf(), inner_file: None };
    }

    let archive_path = Path::new(archive_str);
    if matches!(extensions::from_path(archive_path).as_deref(), Some("zip" | "7z")) {
        ArchivePathSpec {
            archive_path: archive_path.to_path_buf(),
            inner_file: Some(inner_str.to_string()),
        }
    } else {
        ArchivePathSpec { archive_path: path.to_path_buf(), inner_file: None }
    }
}
...
mod tests {
    ...
    #[test]
    fn parse_archive_path_with_inner_file() {
        let spec = parse_archive_path(Path::new(r"E:\games\Sonic.7z#Sonic (U) [!].gen"));
        assert_eq!(spec.archive_path, Path::new(r"E:\games\Sonic.7z"));
        assert_eq!(spec.inner_file.as_deref(), Some("Sonic (U) [!].gen"));
    }

    #[test]
    fn parse_archive_path_without_inner_file() {
        let spec = parse_archive_path(Path::new(r"E:\games\Sonic.7z"));
        assert_eq!(spec.archive_path, Path::new(r"E:\games\Sonic.7z"));
        assert!(spec.inner_file.is_none());
    }

    #[test]
    fn parse_archive_path_hash_in_non_archive_path_is_ignored() {
        let spec = parse_archive_path(Path::new("weird#path.gen"));
        assert_eq!(spec.archive_path, Path::new("weird#path.gen"));
        assert!(spec.inner_file.is_none());
    }
}
