...
impl Console {
    ...
    pub fn from_file(file_path: &Path) -> Option<ConsoleWithSize> {
        let spec = archive::parse_archive_path(file_path);
        if let Some(ref inner_file) = spec.inner_file {
            return Self::from_archive_inner_file(&spec.archive_path, inner_file);
        }

        let file_path = &spec.archive_path;
    ...
    fn from_archive_inner_file(archive_path: &Path, inner_file: &str) -> Option<ConsoleWithSize> {
        let extension = from_path(inner_file)?;
        if let Some(&console) = EXTENSION_LOOKUP.get(&extension.as_str()) {
            return Some(ConsoleWithSize { console, file_size: 0 });
        }

        if extension.as_str() == "bin" {
            let archive_extension = from_path(archive_path)?;
            let contents = match archive_extension.as_str() {
                "zip" => archive::read_file_zip(archive_path, inner_file).ok()?,
                "7z" => archive::read_file_7z(archive_path, inner_file).ok()?,
                _ => return None,
            };
            let console = guess_bin_console_archive(&contents);
            return Some(ConsoleWithSize { console, file_size: contents.len() as u64 });
        }
