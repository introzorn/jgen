...
impl CommonConfig {
    pub(crate) fn read_rom_file(
        ...
        let spec = archive::parse_archive_path(&self.rom_file_path);
        let path = &spec.archive_path;
        let extension = extensions::from_path(path).unwrap_or_default();
        match extension.as_str() {
            "zip" => {
                if let Some(ref inner_file) = spec.inner_file {
                    return Self::read_archive_entry(
                        path,
                        inner_file,
                        supported_extensions,
                        archive::read_file_zip,
                    );
                }
    ...
    fn read_archive_entry(
        ...
        let extension = extensions::from_path(inner_file).ok_or_else(|| {
            NativeEmulatorError::Archive(ArchiveError::unsupported_inner_file(
                archive_path,
                inner_file,
            ))
        })?;
        ...
        if !supported_extensions.contains(&extension.as_str()) {
            return Err(NativeEmulatorError::Archive(ArchiveError::unsupported_inner_file(
                archive_path,
                inner_file,
            )));
        }
        ...
        let contents =
            read_fn(archive_path, inner_file).map_err(NativeEmulatorError::Archive)?;
        Ok(RomReadResult { rom: contents, extension })
    }
}
...
impl AppConfigExt for AppConfig {
    fn common_config(&self, path: PathBuf) -> CommonConfig {
        ...
        CommonConfig {
            ...
            window_x: self.common.window_x,
            window_y: self.common.window_y,
            window_scale_factor: self.common.window_scale_factor,
            audio_output_device: self.common.audio_output_device.clone(),
